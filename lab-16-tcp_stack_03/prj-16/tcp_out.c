...
static void tcp_init_hdr(
        struct tcphdr *tcp, u16 sport, u16 dport,
        u32 seq, u32 ack, u8 flags, u16 rwnd) {
...
// send a tcp packet
//
// Given that the payload of the tcp packet has been filled,
// initialize the tcp header and ip header (remember to set the
// checksum in both header),
// and emit the packet by calling ip_send_packet.
void tcp_send_packet(struct tcp_sock *tsk, char *packet, int len) {
    struct iphdr *ip = packet_to_ip_hdr(packet);
    struct tcphdr *tcp =
            (struct tcphdr *) ((char *) ip + IP_BASE_HDR_SIZE);

    int ip_tot_len = len - (int) ETHER_HDR_SIZE;
    int tcp_data_len = ip_tot_len
                       - (int) IP_BASE_HDR_SIZE - TCP_BASE_HDR_SIZE;
    ...
    tcp_init_hdr(tcp, sport, dport, seq, ack,
                 TCP_PSH | TCP_ACK, rwnd);
    ...
    PushSendBuf(tsk, packet, len);
    ip_send_packet(packet, len);
}

// send a tcp control packet
//
// The control packet is like TCP_ACK, TCP_SYN, TCP_FIN
// (excluding TCP_RST).
// All these packets do not have payload
// and the only difference among these is the flags.
void tcp_send_control_packet(struct tcp_sock *tsk, u8 flags) {
    int pkt_size = ETHER_HDR_SIZE
    ...
    if (flags & (TCP_SYN | TCP_FIN)) {
        tsk->snd_nxt += 1;
        PushSendBuf(tsk, packet, pkt_size);
    }
...
//
// Different from tcp_send_control_packet,
// the fields of reset packet is from tcp_cb instead of tcp_sock.
void tcp_send_reset(struct tcp_cb *cb) {
    ...
    tcp_init_hdr(tcp, cb->dport, cb->sport, 0, cb->seq_end,
                 TCP_RST | TCP_ACK, 0);
    tcp->checksum = tcp_checksum(ip, tcp);
