...
// TCP socks should be hashed into table for later lookup:
// Those which occupy a port (either by *bind* or *connect*)
//   should be hashed into bind_table,
// those which listen for incoming connection request
//   should be hashed into listen_table,
// and those of established connections
//   should be hashed into established_table.
...
// allocate tcp sock, and initialize all the variables
...
// lookup tcp sock in *established_table*
...
static int tcp_port_in_use(u16 sport) {
    ...
    struct tcp_sock *tsk;
    // XXX: framework bug appeared
    // (thanks to Yifan Yang and Jingwen Yang)
    ...
    // Notice: 'bind hash list' is the right data structure
...
// hash tcp sock into
//   either established_table
//   or listen_table according
// to its TCP_STATE
...
//   3. send SYN packet, switch to TCP_SYN_SENT state,
//      wait for the incoming SYN packet by sleep on wait_connect;
//   4. if the SYN packet of the peer arrives,
...
// set backlog (the maximum number of pending connection request),
...
inline int tcp_sock_accept_queue_full(struct tcp_sock *tsk) {
    if (tsk->accept_backlog >= tsk->backlog) {
        log(ERROR, "tcp accept queue (%d) is full.",
            tsk->accept_backlog);
        return 1;
    }
...
inline struct tcp_sock *tcp_sock_accept_dequeue(struct tcp_sock *tsk) {
    struct tcp_sock *new_tsk = list_entry(tsk->accept_queue.next,
...
// if accept_queue is not emtpy,
//   pop the first tcp sock and accept it,
// otherwise,
